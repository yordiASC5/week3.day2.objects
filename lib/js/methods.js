 ...
 */
heroCreator.createCharSheet = function(hero){
    ...
    img.src = hero.img
    div.appendChild(img)
    // append result of createInfoDiv
    div.appendChild(this.createInfoDiv(hero))
    return div
...
heroCreator.createInfoDiv = function(heroObj){
    ...
    for(const key of Object.keys(heroObj)){
        let value = ""
        if(key == "abilities"){
            value = heroObj.getAbilityString(heroObj[key])
        } else {
            value = heroObj[key]
        }
 ...
 * @return {String}
 */
heroCreator.getAbilityString = function(abilitiesArray){
    let formatted = JSON.stringlify()
